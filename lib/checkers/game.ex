defmodule Checkers.Game do
  ...
  def valid_moves(name, user_details, pending_piece) do
    ...
    Enum.reduce(pieces_to_consider, %{}, fn(x, acc) ->
      tiles = if !!pending_piece, do: move_helper(Enum.at(pending_piece, 0), Map.get(game[:tiles], List.first(pending_piece), nil), Enum.drop(List.delete_at(pending_piece, 0), -1), game[:tiles], false), else: game[:tiles]
